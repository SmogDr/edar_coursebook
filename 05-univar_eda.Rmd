...
readr::write_csv(x = noaa_temp, path = "./data/NOAA_temps_cleaned.csv")
```
...
`r round(noaa_temp_avg$temp_avg[1], digits = 1)` $^\circ$F in Colorado
and `r round(noaa_temp_avg$temp_avg[2], digits = 1)` $^\circ$F in
...
`r min(noaa_temp$temp_hr_f)` to `r max(noaa_temp$temp_hr_f)` $^\circ$F!
...
`r format(quantile(ordered_salaries$salary, probs = 0.5), scientific = FALSE, big.mark = ",")`.
