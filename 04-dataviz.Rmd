...
(from the one defined when creating the original `ggplot` object), although this 
is less common. 
```
...
knitr::kable(plot_funcs, 
             ...
             caption = "A Few Basic Geom Types")
```
...
```{r mpg-green, warning=FALSE, message=FALSE, fig.cap="Example of a call to `ggplot` showing counts of vehicle classes from the `mpg` dataframe and colored by a `fill` aesthetic override."}
...
```{r mpg-class-count-alt, eval=FALSE}
# alternative
mpg %>% 
  dplyr::filter(year == 2008) %>% 
  dplyr::count(class)
...
enter its name into the console or script, and it will appear in the Viewer
pane.
```
