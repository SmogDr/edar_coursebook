...
## Ch-9 Homework
```{r ch9-homework, include=FALSE}

#recreate Figure \@ref(fig:log-salary) but show the plot on a log-scale x-axis. Does the histogram appear symmetric when you do this?

#following the code from sal_simulate, Figure \@ref(fig:log-salary) in the form of a cumulative distribution function (instead of a histogram).  Hint: instead of `rlnorm()` you will need to use a different log density function.  Type ?Lognormal into the Console for help.

#mutate the salary_ps2 data to take the log of the salary data.  Then use `fitdistr()` to fit a normal distribution to these data.  What are the resultant parameter estimates for mean and sd?
