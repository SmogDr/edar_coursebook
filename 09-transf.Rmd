...
## Ch-9 Exercises
```{r ch9-exercise, include=FALSE, echo=FALSE}
library(fitdistrplus)
salary_ps <- read_csv("./data/salary_ch9.csv") %>%
  dplyr::filter(salary < 400000,
         salary > 10000) %>%
  dplyr::mutate(log_sal = log(salary))

fit1 <- fitdist(salary_ps$log_sal, distr = "norm")
fit2 <- fitdist(salary_ps$salary, distr = "lnorm")
...
```{r log-salary, include=FALSE, echo=FALSE}

ggplot(data = salary_ps,
       aes(x = log_sal)) +
  geom_histogram(aes(y = ..density..)) +
  stat_function(data = data.frame(x = c(8,14)),
                aes(x),
                fun = dnorm,
                n = length(salary_ps$log_sal),
                args = list(mean = fit1$estimate[[1]], 
                            sd = fit1$estimate[[2]]),
                color = "magenta")
...
```{r log-salary2, include=FALSE, echo=FALSE}
sal_simulate <- tibble(x = rnorm(n = length(salary_ps$salary),
                                 mean = fit1$estimate[[1]],
                                 sd = fit1$estimate[[2]]))


ggplot() +
  geom_histogram(data = salary_ps,
                 aes(x = log_sal),
                 color = "white",
                 bins = 20,
                 alpha = 0.5) +
  geom_histogram(data = sal_simulate,
                 aes(x = x),
                 bins = 20,
                 alpha = 0.5,
                 fill = "blue")
  ...
  #              fun = dlnorm,
  #              n = length(salary_ps$log_sal),
  #             args = list(mean = fit2$estimate[[1]], 
  #                          sd = fit2$estimate[[2]]),
  #              color = "magenta") +
  theme_bw()
