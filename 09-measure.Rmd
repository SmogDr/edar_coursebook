...
```{block type = "rmdnote"}
To collect a simple random sample correctly, you need to ensure three things:  
...
lod_data <- tibble( x_data = c(0.1, 1, 10, 100, 500, 2500),
                    y_data = c(0, 0, 0, 2, 10, 50))

ggplot(data = lod_data) +
...
data_bp <- data_x %>%
  bind_cols(data_y$value2) %>%
  rename(x.data = "value", y.data = "...3")
...
ggplot(data = data_bp) +
  geom_circle(aes(x0 = 0, y0 = 0, r = 1),
   ...
   geom_circle(aes(x0 = 0, y0 = 0, r = 0.1),
              fill = "lightgreen",
              color = "grey") +
  geom_point(aes(x = x.data, y = y.data),
             shape = 4) +
  coord_fixed() +
  theme_no_axes() + 
