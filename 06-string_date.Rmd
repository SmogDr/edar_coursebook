...
stringr::str_detect(string = names_respond, 
                    pattern = "Josh") %>%
  base::sum() / base::length(names_respond)
```
...
```{block, type="rmdnote"}
You will often see time referenced with a **"UTC"**, which stands for
...
[time-zones](https://wikipedia.org/wiki/List_of_tz_database_time_zones) laid
out across Earth's longitude, whereas, **UTC has no timezone**; UTC is the same
