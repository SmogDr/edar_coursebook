# Exploratory Data Analysis: Univariate Data {#eda1}
...
``` {r NOAA_Temps_LDD, echo=FALSE, warning=FALSE, message=FALSE}
NOAA_temps <- read_csv("./data/NOAA_temps.csv") %>%
  mutate(Location = case_when(
    str_detect(STATION, "CJAC") ~ "Colorado",
    str_detect(STATION, "HMAH") ~ "Hawaii")) %>%
  pivot_longer(cols = TAVG:TMIN, 
               names_to = "Temp_type",
               values_to = "Temperature")

ggplot(NOAA_temps, aes(y = Location, x = Temperature, group = Temp_type)) +
  geom_jitter(aes(color = Temp_type),
              height = 0.1) +
  labs(title ="Daily Temperature Values for July, 2019",
       y = "Region")

ggsave("./images/NOAA_temps", device = "png")
  

#Summarize temperature means by station and type
NOAA_temps %>% 
  group_by(STATION, Temp_type) %>%
  summarise(T_averages = mean(Temperature)) -> tmeans

tempdata <- read_csv("./data/NOAA_temps2.csv")
NOAA_temps2 <- tempdata %>%
  mutate(time = str_extract(DATE, "(?<=T).+"), date = str_extract(DATE, ".+(?=T)")) %>%
  select(STATION, NAME, time, date, `HLY-TEMP-NORMAL`) %>%
  rename(station = STATION, name = NAME, temp_hr_f = `HLY-TEMP-NORMAL`)

```

First, if we are picking spots to go camping, both locations have daily high temperatures (TMAX) that seem very pleasant (the average high in July is about `r round(tmeans[[2,3]])` $^\circ$F in Colorado and `r round(tmeans[[5,3]])` $^\circ$F in Hawaii). When we calculate an average value we are creating indicators of a variable's **location**.  However, if we look at the **dispersion** of temperature observations (the span between the daily highs and lows), we can make an important distiction: the range of high-low values in Hawaii is fairly narrow, whereas the range in Colorado spans from `r min(NOAA_temps$Temperature)` to 
`r max(NOAA_temps$Temperature)` $^\circ$F!  
...
``` {r create_cd_plot, warning=FALSE, include=FALSE}
...
ggplot(data = normal_data_cdf, aes(x = a, y = cum_frac)) +
  geom_point(shape = 21,
             fill = "black",
             alpha = 0.25) +
...
ggsave("./normal_cum_dist.png", dpi = 150)
```
...
``` {r quantiles_3, warning=FALSE, message=FALSE}
...
``` {r normal_histogram, warning=FALSE, include=FALSE}
...
``` {r skewed_histogram_outlier, warning=FALSE, include=FALSE}
x1 <- tibble(data = round(rlnorm(5000, meanlog = log(50), sdlog = 0.6), 0))
outliers <- tibble(data = rep(195, 50))
x2 <- bind_rows(x1, outliers)
  
ggplot2::ggplot(data = x2) + 
  geom_histogram(aes(data),
                 bins = 50,
                 fill = "white",
                 color = "darkgrey") +
