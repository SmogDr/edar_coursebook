...
So far, we've shown how to use `dplyr` functions one at a time to clean and
modify data. Often, however, we want to perform several data-wrangling steps
in sequence.

The native R pipe operator, `|>`, provides a convenient way to connect those
steps.

If you look at the format of common `dplyr` functions, you'll notice that
they generally take a data frame as their first argument:
...
mutate(
  .data = dataframe,
  changed_column = function(changed_column),
  new_column = function(other_arguments)
)
```

Without piping, you may repeatedly reassign the data-frame object as each
step is completed:


``` r
daily_show <- read_csv(
  file = "data/daily_show_guests.csv",
  skip = 4
)
...
This works, but the same object must be repeatedly named and reassigned.

### The pipe supplies the first argument

The native pipe `|>` takes the result on its left and supplies it as the
first argument to the function on its right.

For example:

```r
filter(daily_show, category == "Science")
```
...
These two expressions do the same thing. You can think of the pipe as
performing this substitution:

```text
daily_show |> filter(category == "Science")
      ↓
filter(daily_show, category == "Science")
```

Because the pipe supplies the data frame automatically, we do not need to
repeat the data-frame name inside `filter()`.

This same pattern works with the other `dplyr` functions because they also
take the data frame as their first argument.

For example:

```r
select(daily_show, job, category)
```
...
The real utility of the pipe becomes apparent when several operations are
performed in sequence.

The separate cleaning steps above can be written as one pipeline:
...
1. Read the data.
2. Rename several variables.
3. Remove the `YEAR` variable.
4. Convert `job` to lowercase.
5. Retain only observations in the `"Science"` category.

The result of each step becomes the first argument to the next function.
...
Notice that the first argument—the data frame—is omitted from the functions
that follow a pipe. The pipe supplies the result of the previous step as that
argument automatically.

For now, think of the pipe as a way to make code read in the same order that
you think through an analysis. As your analyses become more complex, this
becomes extremely powerful: you can connect many data-processing steps into
one clear, readable workflow without repeatedly naming intermediate objects
or nesting functions inside one another.

### Order matters

The order of operations in a pipeline matters.

For example, if you remove a variable using `select()` and then try to use
that variable later in the pipeline, R will return an error because the
variable no longer exists in the object being passed forward.

When learning to build pipelines, it can be helpful to run the code one step
at a time and inspect how the data frame changes after each operation.

### Formatting pipelines

R generally does not require a new line after each pipe, but putting each
operation on its own line makes pipelines much easier to read.
...
```r
daily_show |> filter(category == "Science") |> select(job, guest_name)
```

Both work, but the first is easier to read and debug.

If a function contains several arguments, placing those arguments on separate
lines can also improve readability.

### Pipe keyboard shortcut

RStudio provides a keyboard shortcut for inserting a pipe:

* **Mac:** `Shift + Command + M`
* **Windows:** `Ctrl + Shift + M`

In current versions of RStudio, this shortcut can be configured to insert
the native R pipe `|>`.

If the shortcut inserts `|>` instead, go to **Tools → Global Options →
Code → Editing** and select **Use native pipe operator, `|>`**.
...
## Interactive tutorial: Working with Data

Before beginning the Chapter 3 exercises, complete the **Working with Data** tutorial from the `edarlearn` package.

In RStudio, open the **Tutorial** tab and select:

**edarlearn → Working with Data → Start Tutorial**

This tutorial builds on *Working in R* and introduces the basic structure of R datasets and several core `dplyr` tools. You will practice:

- recognizing common R data classes,
- understanding vectors, data frames, and tibbles,
- distinguishing observations (rows) from variables (columns),
- inspecting datasets,
- renaming variables with `rename()`,
- selecting variables with `select()`,
- filtering observations with `filter()`,
- sorting observations with `arrange()`,
- creating variables with `mutate()`, and
- combining these operations with the native R pipe `|>`.

Your work is saved automatically. Use **Start Over** at the bottom of the tutorial navigation panel if you want to reset the entire tutorial.

If the tutorial does not appear in RStudio's Tutorial pane, you can launch it from the Console:


``` r
learnr::run_tutorial("working-with-data", package = "edarlearn")
```
