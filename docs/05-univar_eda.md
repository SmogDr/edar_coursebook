...
simple: learn about your data through diagnostic visualization.
...
familiarize oneself with a dataset. As a result, you can use EDA to do many
...
*Univariate* means "only one variable" being considered. Just like medical
...
still useful. Trust me for now, or skip ahead to multivariate analyses---your
choice.
...
The location of univariate data means: where do most of the values or
...
*probability density function*. It is important to know the "shape" of the
distribution because, once you know a distribution's shape, you can model it. 
And if you can model the distribution, you can begin to make inferences about
it (e.g., extrapolations, predictions). More on this later in
[**Modeling**](#model).
...
2010. We can download such data from the
...
In Figure \@ref(fig:noaa-temp-plot), we use `geom_jitter()` to plot the hourly
...
*location*. But if we look at the *dispersion* of temperature observations
...
more layers if you want to be comfortable in Colorado. See Figure
\@ref(fig:noaa-temp-anno).

You will discover that there are many ways to communicate location (e.g., mode,
mean, median) and dispersion (e.g., range, interquartile range, standard
deviation). We will discuss several such descriptors in this course.
...
In other words, quantiles break a set of observations into similarly sized
...
There are two important aspects about quantiles to remember: (1) each quantile
is defined only by its upper-end value; and (2) quantiles are defined after
...
- Minimum (`min()` in R): the 0% or lowest value; the zeroth quantile

- Maximum (`max()` in R): the 100^th^ percentile (or 1.0 in fractional terms); the highest value observed; the n^th^ quantile   
...
the mean (`mean()` in R) or standard deviation (`sd()`) in this list. It is not
because I dislike those descriptors; rather, it is because their use implies
that one knows the type or "shape" of the distribution being described (e.g.,
normal, log-normal, bi-modal). Use of descriptors like mean and standard
deviation are very useful when applied properly but can be misleading when the
distribution is skewed (more on that later). 
...
Now let's create quantiles from a *normal distribution* of data with a mean of
50 and standard deviation of 15. We'll start by randomly sampling 1000 values,
using `rnorm()`, and then arranging them with the `quanitle()` function. Note
that the second half of the code chunk below shows how to calculate quantiles
manually.
...
show" ALL of the data without modification. This type of plot shows the all,
raw data in a single plot. There are strengths and weaknesses to both these
approaches.
...
Engineering, you have a good chance of making a great living wage. The median
salary is $
...
One interesting thing you may notice in Figure \@ref(fig:cdf-me-salaries) is
that the "tails" of the plot (i.e., the shape of the curves about the median
value) are not symmetrical. One tail looks longer than the other. This lack of
symmetry implies a degree of *skewness* in the data. Why is that? We will
explore skewness at several points in this course and discuss some of the
...
delve into managing outliers later on; for now, it's enough to know that they
exist. 
...
deviation" to the mean of the data in Figure \@ref(fig:skew-hist-anno-2), we
...
variability tend to produce skewed data---log-normal, in the case of Figure
...
that suggest $x = a * b * c$ not $x = a + b + c$. Explicit examples of
...
respectively, or a distance of $1.5 * IQR$ if either the maximum or minimum 
value are larger than $1.5 * IQR$.  Other times the whiskers represent the 0.9
or 0.95 quantiles or $1.58 * IQR \div \sqrt(2)$. Because of these variations,
it's best to define the whiskers for the viewer either in the text or the
Figure caption.  

Here is the R definition from the helpfile `?geom_boxplot`:
...
its advantages. For example, let's look at the data from Figure \@ref(fig:noaa-temp-plot) as two boxplots.

<div class="figure" style="text-align: center">
<img src="05-univar_eda_files/figure-html/temp-boxplot-1.png" alt="Boxplots of hourly temperatures in Colorado and Hawaii for July, 2010. Whiskers represent 1.5*IQR." width="700pt" />
<p class="caption">(\#fig:temp-boxplot)Boxplots of hourly temperatures in Colorado and Hawaii for July, 2010. Whiskers represent 1.5*IQR.</p>
</div>
...
\@ref(fig:noaa-temp-plot) as a time series. We will use  `geom_point()` and
`geom_line()` to create this plot, both of which require the following
aesthetics: `x = ` and `y = `.
