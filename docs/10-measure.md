...
Let's review a few common sampling strategies below.  
...
The LOD is analogous to the concept of **signal to noise** ratio for continuous sensor data (e.g., temperature, pressure, current/voltage, etc.) and the equation above can be used to estimate LOD for any sensor, provided you can extract the a "background" signal from the data.  In this case, the mean and standard deviation of your background "noise" are used for $\mu_b$ and $\sigma_b$, respectively.  

The limit of quantification, *LOQ*, is used as a threshold of confidence to indicate that, not only can you detect something (from nothing), but that you can ***quantify how much***.
