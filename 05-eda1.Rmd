...
``` {r noaa-temp-data, echo=FALSE, warning=FALSE, message=FALSE}
...
noaa_temp %>% 
  ...
  geom_jitter(height = 0.15,
              size = 1.5,
              shape = 1) +
  labs(title = "Comparing location and dispersion of hourly July temperatures",
       y = "Region",
       x = "Temperature (°F)") +
  theme_bw() +
...
to `r max(noaa_temp$temp_hr_f)` $^\circ$F in July 2010! 
...
norm_manual <- tibble::tibble(
  ...
  sample_data = norm_quant, 
  # sort the data
  sorted_data = sort(norm_quant),  
  # calculate cumulative fraction for each entry
  cum_frac = seq.int(from = 1 / length(norm_quant), 
                     to = 1,
                     by = 1 / length(norm_quant)))
...
sample.norm<-tibble(x.data = rnorm(10000, mean = 30, sd = 3))

#plot the histogram
ggplot(data = sample.norm) + 
  geom_histogram(aes(x = x.data),
                 bins = 30,
                 fill = "white",
                 color = "darkgrey") +
  theme_minimal() +
...
set.seed(2)
data.frame.2 <- tibble(data = round(rlnorm(5000, meanlog = log(50), sdlog = 1), 0))
outliers <- tibble(data = rep(195, 75))
data.frame.3 <- bind_rows(data.frame.2, outliers)
  
ggplot(data = data.frame.3) + 
...
* mode: `r names(table(data.frame.3)[which.max(table(data.frame.3))])`  
* median: `r median(data.frame.3$data)`  
* mean: `r round(mean(data.frame.3$data), 0)`  
...
ggplot(data = noaa_temp, aes(y = temp_hr_f, 
                              x = location, 
                              group = location)) +
...
```
Examination of the boxplots in Figure \@ref(fig:boxplot-2) conveys much the same information as what is seen for the raw data originally plotted in Figure \@ref(fig:noaa-temp-plot): both distributions have same central tendencies but the Colorado distribution has considerably more variation.
...
Let's plot the same temperature data from Hawaii as shown in Figure \@ref(fig:noaa-temp-plot) as a time series.  We will use  `geom_point` and `geom_line` to create this plot, both of which require the following aesthetics: `x = ` and `y = `
```{r NOAA-time-series, warning=FALSE, message=FALSE}
# plot Temp vs Time for Colorado data
NOAA_HI_time_series<- ggplot(data = filter(noaa_temp, location == "Hawaii"), 
       aes(x = date, 
           y = temp_hr_f)) +
...
# plot Temp vs Time for Colorado data
NOAA_HI_time_series<- ggplot(data = filter(noaa_temp, location == "Hawaii", date <= "2010-07-05"), 
       aes(x = date, 
           y = temp_hr_f)) +
  geom_line(linejoin = "round",
            color = "purple") +
...
Let's examine the Hawaii temperature data shown in Figures \@ref(fig:noaa-temp-plot) and \@ref(fig:NOAA-HI-time-series) using an autocorrelation plot. The `ggplot2` package doesn't have explicit geoms for autocorrelation plots (sigh), but there are simple alternatives.
...
pacf(noaa_temp_HI$temp_hr_f,      #call the partial autocorrelation plot
     main = " ", 
     xlab = "Lag (hours)",
