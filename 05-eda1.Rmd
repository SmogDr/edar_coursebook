...
more layers if you want to be comfortable in Colorado. See
...
normal_data_cdf <- normal_data %>%
  select(a) %>%
  arrange(a) %>%
  mutate(unit_quantiles = seq.int(from = 1/length(a),
                            to = 1,
                            by = 1/length(a)))
  
#plot cumulative distribution
normal_data_cdf_plot <- ggplot(data = normal_data_cdf, 
                          aes(x = a, y = unit_quantiles)) +
  geom_point(shape = 21,
             fill = "black",
             alpha = 0.25) +
...
Let's plot the same temperature data from Hawaii as shown in \@ref(fig:noaa-temp-plot) as a time series.  We will use  `geom_point` and `geom_line` to create this plot, both of which require the following aesthetics: `x = ` and `y = `
